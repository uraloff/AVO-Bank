...
from App.Rag.bot_brain import AVOBrain
from App.Core.Database.Requests import user_rq
from App.Bot.Keyboards import kb_keyboards as kb
from App.Bot.States.asking_question import AskingQuestion
...
# ------------------------------ КНОПКА "Русский язык" ------------------------------
@ru_user_router.callback_query(F.data == 'ru')
async def ru_user(callback: CallbackQuery, state: SendingPhoneNumber) -> None:
    await user_rq.set_user_language(callback.from_user.id, 'ru')
    await callback.answer("Выбран русский язык 🇷🇺")
    await callback.message.delete()
    await callback.message.answer(
        "Отлично! Чтобы продолжить дальше отправьте свой номер телефона нажав на кнопку ниже 👇",
        reply_markup=kb.send_phone_kb
    )
    await state.set_state(SendingPhoneNumber.sending_phone_number)


@ru_user_router.message(SendingPhoneNumber.sending_phone_number)
async def ru_sending_phone_number(message: Message, state: SendingPhoneNumber) -> None:
    if not message.contact:
        await message.answer("Пожалуйста, отправьте номер телефона, нажав на кнопку ниже 👇")
        return
    
    await user_rq.set_user_phone_number(message.from_user.id, message.contact.phone_number)
    
    wait_msg = await message.answer("⏳", reply_markup=ReplyKeyboardRemove())
...
async def ru_bonuses(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()
    await callback.message.delete()

    sent_message = await callback.message.answer_photo(
        photo=img.about_bonus_ru_photo,
        caption=(
            "<b>Как получить 25 000 бонусных сум?</b>\n"
...
async def ru_card(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()
    await callback.message.delete()

    sent_message = await callback.message.answer_photo(
        photo=img.about_card_ru_photo,
        caption=(
            "✅ <b>До 50 000 000 сумов</b>\n"
...
async def ru_thirty(callback: CallbackQuery, state: FSMContext) -> None:
    try:
        await callback.answer()

        sent_message = await callback.message.edit_text(
            "<b>30 дней на покупки</b>\n\n"
...
async def ru_fifteen(callback: CallbackQuery, state: FSMContext) -> None:
    try:
        await callback.answer()

        sent_message = await callback.message.edit_text(
            "<b>Ещё 15 дней на оплату</b>\n\n"
...
async def ru_payment_time(callback: CallbackQuery, state: FSMContext) -> None:
    try:
        await callback.answer()

        sent_message = await callback.message.edit_text(
            "<b>Покупаете раньше — больше времени на оплату</b>\n\n"
...
async def ru_about_us(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()
    await callback.message.delete()

    sent_message = await callback.message.answer_photo(
        photo=img.about_us_ru_photo,
        caption=(
            "<b>Ценности</b>\n\n"
...
async def ru_documents(callback: CallbackQuery, state: FSMContext) -> None:
    try:
        await callback.answer()
        
        sent_message = await callback.message.edit_text(
            "<b>Документы и лицензии</b>\n\n"
...
async def ru_for_clients(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()
...
async def ru_main_documents(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()
...
async def ru_important_facts_1(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()

    sent_message = await callback.message.edit_text(
        "<b>Существенные факты</b>\n\n"
...
async def ru_important_facts_2(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()
...
async def ru_internal_regulatory_documents_1(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()

    sent_message = await callback.message.edit_text(
        "<b>Внутренние нормативные документы</b>\n\n"
...
async def ru_internal_regulatory_documents_2(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()
...
async def ru_affylated_persons(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()

    sent_message = await callback.message.edit_text(
        "<b>Аффилированные лица</b>\n\n"
...
async def ru_application_form(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()
...
async def ru_financial_statements(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()
...
async def ru_audit_reports(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()

    sent_message = await callback.message.edit_text(
        "<b>Аудиторские заключения</b>\n\n"
        "📄 <a href='https://docs.avo.uz/AVO_financial_statements.pdf'>Годовой отчёт по итогам 2023 года</a>",
        reply_markup=inline_builder(
            ['Назад ↩️'],
            ['ru_back_to_documents_from_audit_reports']
...
async def ru_dividends(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()
...
async def ru_operations(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()

    sent_message = await callback.message.edit_text(
        "<b>Переводы и снятие наличных</b>\n\n"
...
async def ru_other_tariffs_1(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()

    sent_message = await callback.message.edit_text(
        "<b>Услуги, связанные с AVO Bank</b>\n\n"
...
async def ru_other_tariffs_2(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()

    sent_message = await callback.message.edit_text(
        "<b>Обслуживание карт сторонних банков</b>\n\n"
...
async def ru_giveaway(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()
    await callback.message.delete()

    sent_message = await callback.message.answer_photo(
        photo=img.about_giveaway_ru_photo,
        caption=(
            "<b>🌟 Розыгрыш «AVO айфон марафон» 🌟</b>\n\n"
...
@ru_user_router.message(F.text == "📝 Задать вопрос")
async def ru_ask_question(message: Message, state: AskingQuestion) -> None:
    wait_msg = await message.answer("📝", reply_markup=kb.connect_operator_kb)
    
    await wait_msg.delete()
    await message.answer(
        "Задайте свой вопрос или выберите из списка ниже.\n"
        "Например: <i>«Как увеличить шансы получения кредитного лимита?»</i> или <i>«Как погасить задолженность?»</i>",
        reply_markup=inline_builder(
            [
                "Розыгрыш «AVO айфон марафон»", "Как получить кредитные деньги?", "Как получть карту?",
                "Как пользоваться кредитным лимитом?", "Какие тарифы?", "Как заработать с AVO?"
            ],
            [
                "ru_giveaway", "ru_how_to_get_loan_money", "ru_how_to_get_card",
                "ru_how_to_use_credit_limit", "ru_which_tariffs", "ru_how_to_earn_with_avo"
            ]
        )
    )
    await state.set_state(AskingQuestion.asking_question)


@ru_user_router.message(AskingQuestion.asking_question)
async def ru_receive_question(message: Message, state: AskingQuestion) -> None:
    await message.bot.send_chat_action(chat_id=message.chat.id, action="typing")
    
    answer = await rag_brain.get_answer(message.text)
    
    if answer:
        # RAG нашел ответ
        await message.answer(answer)
    else:
        # RAG не уверен -> переключаем на человека
        await message.answer("Извините, я пока не нашел точного ответа на этот вопрос в моих инструкциях. Соединяю с оператором... 👨‍💻")
        await state.clear()